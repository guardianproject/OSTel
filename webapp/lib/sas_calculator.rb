 ...
 * Copyright (C) 2013 Twelvetone Software <lee@twelvetone.info>
 *
 ...
 * out of the ZRTP handshake. Ported from Java to Ruby by Lee Azzarello.
 *
...
module SasCalculator 
  ...
  # argument must be an array of bytes of size 4
  # the algorithm only takes the first two bytes and maps them to the
  # dictionaries. Weird. It could be four words but does it matter?
  def calculateSAS(sasBytes)
    # scale the range of each byte to integers between 0..255 AKA 8 bit ASCII
    wordIndexOne = sasBytes[0].to_i;
    wordIndexTwo = sasBytes[1].to_i;
